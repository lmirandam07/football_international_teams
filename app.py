...
def get_results(df, team):
    ...
    df['wins'] = np.where(((df.home_score > df.away_score) & (df.home_team == team) | (
        df.home_score < df.away_score) & (df.away_team == team)), 1, 0)

    df['draws'] = np.where(((df.home_score == df.away_score) & (df.home_team == team) | (
        df.home_score == df.away_score) & (df.away_team == team)), 1, 0)

    df['loses'] = np.where(((df.home_score < df.away_score) & (df.home_team == team) | (
        df.home_score > df.away_score) & (df.away_team == team)), 1, 0)
...
def create_sidebar(teams, df):
    with st.sidebar:
        ...
        df = df[df['home_team'].isin(
            teams) | df['away_team'].isin(teams)]

        team = st.selectbox('Countries', teams)

        team_cond = (df['home_team'] ==
                     team) | (df['away_team'] == team)
        ...
        df = df[(df.date >= start_date) &
                (df.date <= end_date)].sort_values(by="date", ascending=False)

        # Top N df from the selected input
        n_matches = st.number_input(
            "Number of matches", min_value=1, max_value=len(df), value=10, help="This will limit the number of rows on the dataset")
...
def main():
    ...
    wc_teams: list = ['Qatar', 'Ecuador', 'Senegal', 'Netherlands', 'England', 'Iran', 'United States', 'Wales',
                      'Argentina', 'Saudi Arabia', 'Mexico', 'Poland', 'France', 'Australia', 'Denmark',
                      'Tunisia', 'Spain', 'Costa Rica', 'Germany', 'Japan', 'Belgium', 'Canada', 'Morocco',
    ...
    team_results = matches.groupby(by='selected_team', as_index=False)[
        ['wins', 'loses', 'draws', 'goals_scored', 'goals_conceded', 'goals_diff']].sum()
    ...
    st.markdown(
        ...
        and <code style='color:#e34234'>lost</code> **{team_results['loses'][0]}** of its last **{len(matches)}** games""", unsafe_allow_html=True)

    cond_color = "<code style='color:#66cd00'>" if team_results[
        'goals_diff'][0] > 0 else "<code style='color:#e34234'>"
    ...
    st.markdown(
        ...
        making up a difference of {cond_color}**{team_results['goals_diff'][0]}**</code> goals""", unsafe_allow_html=True)

    bar_plot = px.bar(matches, y=['wins', 'loses', 'draws'], x='selected_team')
    bar_plot = bar_plot.update_xaxes(type='category')
